...
local function brushSettingsSlider_terraforming()
  ...
  if editor.selection and editor.selection.object and editor.selection.object[1] then
    if editor.uiIconImageButton(editor.icons.terrainToTwoLines, nil, nil, nil, nil, 'terraform2SelectionBtn') then
  ...
  if terraformer.isPolygonExist() then
    if editor.uiIconImageButton(editor.icons.wallpaper, nil, nil, nil, nil, 'terraform2PolygonBtn') then
...
Applies to 'texture size', 'macro texture size' and 'detail texture size'.
  ...
  prefsRegistry:registerSubCategory("terrainEditor", "terraform", nil, {
    {terraformDOI = {"int", 100, "Terraform Domain of Influence (m)", nil, 1, 500}},
    {terraformMargin = {"float", 5.0, "Terraform Margin (m)", nil, 0.0, 20.0}},
  })
end
