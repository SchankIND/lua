...
local function recalculateRaceDistance()
  local aiPath, aiPathDetailed = currentPath:getAiPath()
  local dist = 0
  for i = 1, #aiPathDetailed - 1 do
    local point = aiPathDetailed[i]
    local nextPoint = aiPathDetailed[i+1]
    if nextPoint then
      dist = dist + point.pos:distance(nextPoint.pos)
    end
  end
  cachedRaceDistance = dist
end
...
local function raceDistanceString()
  if cachedRaceDistance < 0 then
    return "Tools>Recalculate Race Distance to recalculate distance"
  else
  ...
  if editor.beginWindow(toolWindowName, "Race Tool", im.WindowFlags_MenuBar) then
    if im.BeginMenuBar() then
      ...
      if im.BeginMenu("Tools") then
        ...
        if im.MenuItem1("Recalculate Race Distance") then
          recalculateRaceDistance()
        end
...
local function onEditorRegisterPreferences(prefsRegistry)
  ...
  prefsRegistry:registerSubCategory("raceEditor", "general", nil,
  {
    ...
    {showCustomFields = {"bool", false, "Display custom field values next to the node name in the world."}},
    {useSimpleDrag = {"bool", false, "Use simple drag mode for the race editor."}},
  })
end
