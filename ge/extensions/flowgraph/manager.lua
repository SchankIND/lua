...
function C:updateEditorSelections()
  ...
  -- nodes
  local selectNodeIdArray
  local startIdx = IMGUI_LUAINTF and 1 or 0
  if IMGUI_LUAINTF then
    selectNodeIdArray = ui_flowgraph_editor.GetSelectedNodeIds()
    self.selectedNodeCount = #selectNodeIdArray
  else
    selectNodeIdArray = ffi.new('fge_NodeId[' .. tostring(selectCountMax) .. ']')
    self.selectedNodeCount = ui_flowgraph_editor.GetSelectedNodes(selectNodeIdArray, selectCountMax)
  end
  self.selectedNodes = {}
  for i = startIdx, self.selectedNodeCount - 1 + startIdx do
  ...
  -- links
  local selectLinkIdArray
  if IMGUI_LUAINTF then
    selectLinkIdArray = ui_flowgraph_editor.GetSelectedLinkIds()
    self.selectedLinkCount = #selectLinkIdArray
  else
    selectLinkIdArray = ffi.new('fge_LinkId[' .. tostring(selectCountMax) .. ']')
    self.selectedLinkCount = ui_flowgraph_editor.GetSelectedLinks(selectLinkIdArray, selectCountMax)
  end
  self.selectedLinks = {}
  for i = startIdx, self.selectedLinkCount - 1 + startIdx do
