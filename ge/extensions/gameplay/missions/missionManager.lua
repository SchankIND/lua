...
local function taskStartVehicleStep(step)
  ...
  if not vehicleSetup.enabled then
    ...
    if vehicleSetup.usePlayerVehicle or not vehicleSetup.vehicles or not idx then -- if selection does not exist, use player vehicle
      ...
      local veh = getObjectByID(taskData.data.mission._startingInfo.vehId)
      if veh then
      ...
      if career_career.isActive() then
        ...
        if inventoryVehId then
          local class = career_modules_vehiclePerformance.getVehicleClass(inventoryVehId)
          if class then
            taskData.data.mission._startingInfo.vehicleClass = class
          end
        end
      end
