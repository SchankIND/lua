...
-- poolingMode = mode that limits the amount of active vehicles (global uses all vehicles in the scene)
...
function VehPool:new(data)
  ...
  new.poolingMode = data.poolingMode or "default" -- default, global
...
function VehPool:checkActiveLimit()
  ...
  local activeAmount = self.poolingMode == "default" and #self.activeVehs or self:getSceneActiveAmount()
...
function VehPool:activateByDistanceTo(pos, dist)
  -- this can be called from onUpdate in modules
