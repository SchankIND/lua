...
local abs = math.abs
local userpathCaseSensitive = false
...
local function onUiReady()
  ...
  if Engine.Platform.getOSInfo().type == 'linux' then
    userpathCaseSensitive = FS:isUserPathCaseSensitive() == 1
  end
...
local function packMod(dirPath)
  ...
  if userpathCaseSensitive then
    local errorMsg = 'User path \'' .. FS:getUserPath() .. '\' is on a case-sensitive filesystem, please pack the files manually instead.'
    log('E', 'packMod', errorMsg)
    guihooks.trigger('modmanagerError', 'Unable to pack mod: ' .. errorMsg)
    return false
  end
